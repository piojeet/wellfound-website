...
(function () {
  ...
  document.write(
    '<style type="text/css">.marquee{white-space:nowrap;overflow:hidden;visibility:hidden;}' +
  ...
  if (ie && iever < 8) {
    ...
    window.attachEvent("onload", function () {
      ...
      while (++i < limit) marqueeInit.run(marqueeInit.table[i]);
    });
  }
  ...
  function Marq(c, tag) {
    ...
    if (this.mq.random) {
      if (
        ...
        tag.getElementsByTagName("tr").length === 1 &&
    ...
    this.mq.direction =
      this.mq.persist && this.cookie.get(this.mq.uniqueid)
        ? this.cookie.get(this.mq.uniqueid).split(":")[2]
        : this.mq.direction;
    this.mq.style.width =
    ...
    this.m.style.alignItems = "center";
    this.m.style.gap = "15vh";
    if (ie5)
  ...
  Marq.prototype.setup = function (c) {
    ...
    this.m[0].style.left =
      this.mq.persist && this.cookie.get(this.mq.uniqueid)
        ? this.cookie.get(this.mq.uniqueid).split(":")[0]
        : 0;
    ...
    this.m[1].style.left =
      this.mq.persist && this.cookie.get(this.mq.uniqueid)
        ? this.cookie.get(this.mq.uniqueid).split(":")[1]
        : this.w + "px";
  ...
  Marq.prototype.cookie = {
    set: function (n, v, d) {
      // cook.set takes (name, value, optional_persist_days) - defaults to session if no days specified
      if (d) {
        var dt = new Date();
    ...
    kill: function (n) {
      // cook.kill takes (name)
      cook.set(n, "", -1);
    },
    killall: function () {
      // cook.killall takes no parameters
  ...
  function unload() {
    for (var m, i = 0; i < marqueeInit.ar.length; ++i) {
      if (
        marqueeInit.ar[i] &&
        marqueeInit.ar[i].persist &&
        marqueeInit.ar[i].setup
      ) {
        m = marqueeInit.ar[i].setup;
        m.cookie.set(
          m.mq.uniqueid,
          m.m[0].style.left + ":" + m.m[1].style.left + ":" + m.mq.direction
        );
      }
    }
  }
